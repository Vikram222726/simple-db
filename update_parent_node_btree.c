...
void internal_node_insert(Table* table,uint32_t parent_page_num,uint32_t new_page_num){
    ...
    uint32_t child_node_max_key = get_node_max_key(new_child_node);
    ...
    *(internal_node_num_keys(parent_node)) += 1;
    ...
    if(num_keys_in_parent >= INTERNAL_NODE_MAX_CELLS){
        printf("Need to implement code to split internal node!");
        exit(EXIT_FAILURE);
    }else{
        ...
        uint32_t rightmost_node_max_key = get_node_max_key(rightmost_node);
